 ...
 */
class InlineEdit extends \Magestore\RewardpointsRule\Controller\Adminhtml\Earning\Individual
{
    /**
     * @return $this
     */
    public function execute()
    {
        ...
        foreach (array_keys($postItems) as $productId) {
            if(isset($postItems[$productId]['entity_id']) && $postItems[$productId]['entity_id'] && isset($postItems[$productId]['rewardpoints_earn']))
            {
                $model->load($postItems[$productId]['entity_id']);
                $model->setRewardpointsEarn($postItems[$productId]['rewardpoints_earn']);
                $model->save();
            }
        }
